...
function getDateForStatusUpdate(request: ServiceRequestItem): Date {
    if (request.requestedDateTime) {
        const d = new Date(request.requestedDateTime);
...
function ServiceListing() {
    ...
    /** `${requestId}:${action}` while that mutation is in flight */
    ...
    useEffect(() => {
        if (!openPciker) setPendingRequestId(null);
    }, [openPciker]);
    ...
    useEffect(() => {
        if (servicesRequests?.data) {
            const filtered = servicesRequests.data.filter((request: ServiceRequestItem) => {
                if (activeStatusTab === "incoming") {
    ...
    }, [servicesRequests, activeStatusTab]);
    return (
        <div className="flex flex-col min-h-screen w-full max-w-full min-w-0 overflow-x-hidden p-3 sm:p-4 lg:p-6">
            ...
            >
                <DateTimePickerModal
                    ...
                    isLoading={
                        !!pendingRequestId &&
                        pendingMutationKey === `${pendingRequestId}:propose`
                    }
                    setStep={() => { }}
                    setDate={setDate}
                    onConfirm={
                        pendingRequestId
                            ? async (d) => {
                                await handleUpdateServiceRequest(
                                    pendingRequestId,
                                    d,
                                    "propose"
                                );
            ...
            <div className="flex flex-col lg:flex-row flex-1 min-h-0 w-full min-w-0 lg:max-h-[calc(100dvh-140px)]">
                    ...
                    <div className="px-3 sm:px-4 lg:px-6 pt-3 sm:pt-4 flex-1 min-h-0 overflow-y-auto">
                        {(!isHydrated || isLoading || isFetching) ? (
                            ...
                            filteredServicesRequests.map((request, index) => {
                                ...
                                const rowBusy =
                                    !!reqId &&
                                    pendingMutationKey?.startsWith(`${reqId}:`);
                                const rowAwaitingOfferModal =
                                    !!reqId &&
                                    openPciker &&
                                    pendingRequestId === reqId;
                                const actionsDisabled =
                                    !reqId || rowBusy || rowAwaitingOfferModal;
                                const loading = (action: "accept" | "reject" | "propose") =>
                                    !!reqId && pendingMutationKey === `${reqId}:${action}`;
                                const statusPending =
                                ...
                                return (
                                    ...
                                    >
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p className="text-[15px] font-medium mt-3 leading-snug text-black-1 break-words">
                                                {/* {ph(offer.dateKey)} */}
                                        ...
                                        {statusPending && (
                                            <div className="w-full shrink-0 xl:w-[297px] lg:max-w-full">
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        void handleUpdateServiceRequest(
                                                            reqId,
                                                            getDateForStatusUpdate(request),
                                                            "accept"
                                                        );
                                                    }}
                                                ...
                                                <div className="mt-2 flex items-center gap-[9px]">
                                                    ...
                                                    >
                                                        {loading("reject") ? (
                                                            <BeatLoader color="#129A8E" size={8} />
                                                        ) : (
                                                            ph("decline")
                                                        )}
                                                    </button>
